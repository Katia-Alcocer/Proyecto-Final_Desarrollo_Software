...
function insertarEmpleado($pdo, $nombre, $apellido_p, $apellido_m, $telefono, $curp, $rfc, $idPuesto, $idSucursal) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ObtenerEmpleados.php?mensaje=Empleado agregado con éxito.");
        exit;
    } else {
        // Redirigir con mensaje de error
        header("Location: ObtenerEmpleados.php?mensaje=Error al agregar el empleado.");
        exit;
    }
}
...
function obtenerEmpleados($pdo, $estatus) {
    ...
    if ($idSucursal) {
        ...
        WHERE e.estatus = :estatus AND e.idSucursal = :idSucursal";
        ...
        return []; // Si no hay un idSucursal en la sesión, retorna un arreglo vacío
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $idSucursal = $_SESSION['idSucursal']; // Usar idSucursal de la sesión
        ...
        foreach ($empleados as $empleado) {
            ...
            echo "<td>
             <span>
                    ...
                    <a href='Eliminar_emp.php?id=" . htmlspecialchars($empleado['idEmpleado']) . "' onclick=\"return confirm('¿Estás seguro de que deseas eliminar este empleado?');\">
...
<button onclick="window.location.href='../pagina_sucursar.html';">Regresar</button>
<br>
<button onclick="window.location.href='AgregarEmplea.php';">Agregar Empleado</button>
