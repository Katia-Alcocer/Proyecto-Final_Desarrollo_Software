...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ObtenerEmpleados.php?mensaje=Empleado Eliminado.");
        exit;
    } else {
        // Redirigir con mensaje de error
        header("Location: ObtenerEmpleados.php?mensaje=Error al eliminar Empleado.");
        exit;
    }
} else {
    // Redirigir con mensaje de error si el ID no es válido
    header("Location: ObtenerEmpleados.php?mensaje=ID de empleado no válido.");
    exit;
}
?>