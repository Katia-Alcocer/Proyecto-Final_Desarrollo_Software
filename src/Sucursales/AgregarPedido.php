...
// Verificar si existe la sucursal en la sesión
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($medicamentos) || empty($cantidades)) {
        ...
        try {
            ...
            $stmt->execute([$idSucursalSesion, $idProveedor, $fechaPedido, $estado]);
...
<body>
    ...
    <form method="POST">
        ...
        <input type="text" value="<?= $sucursales[array_search($idSucursalSesion, array_column($sucursales, 'idSucursal'))]['Nombre'] ?>" disabled><br><br>
        
        <input type="hidden" name="idSucursal" value="<?= $idSucursalSesion ?>">
        ...
        <button type="button" onclick="window.location.href='../pagina_sucursar.html';">Regresar</button>
    </form>
</body>
</html>
